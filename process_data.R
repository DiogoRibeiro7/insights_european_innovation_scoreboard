...
# Summarize the number of NaN values in each column
sum_nan_per_column <- sapply(dados_with_na, function(x) sum(is.nan(x)))

# Print summaries
print(sum_na_per_column)
print(sum_nan_per_column)

# Summarize unique values in each column to identify potential non-NA missing values
unique_values_per_column <- sapply(dados_with_na, unique)
print(unique_values_per_column)

# Convert "n/a" to NA only in character columns
dados_with_na <- dados_with_na %>%
